...
export default function AASResultsPage() {
  const router = useRouter();
  const [score, setScore] = useState(0);
  const [total, setTotal] = useState(50);

  useEffect(() => {
    const savedState = localStorage.getItem('quiz-aas');
    if (savedState) {
      try {
  ...
  const getFeedback = () => {
    if (percentage >= 90) return { message: 'Excellent! You have an outstanding understanding of AAS!', color: 'text-emerald-600', bgColor: 'bg-emerald-50', borderColor: 'border-emerald-200' };
    if (percentage >= 75) return { message: 'Great job! You have a solid understanding of AAS concepts.', color: 'text-blue-600', bgColor: 'bg-blue-50', borderColor: 'border-blue-200' };
  ...
  const handleRetry = () => {
    localStorage.removeItem('quiz-aas');
    router.push('/aas/quiz');
  };
      ...
      <div className="relative z-10 flex items-center justify-center px-4 py-6 sm:py-8">
        ...
        >
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-5 sm:p-8 text-center">
            ...
            <p className="text-[#666666] text-xs sm:text-sm mb-6">AAS Machine Assessment</p>
